...
export class Account {
         ...
         static createAccount(password: string, label?: string) {
           ...
           account.encryptedKey = privateKey.encrypt(
             password,
             address,
             salt
           );
          ...
          * Import account
          */
         static importAccountByPrivateKey(
           password: string,
           privateKey: string,
           label?: string
         ): Account {
          ...
          * Import account
          */
         static importAccountByWIFPrivateKey(
           password: string,
           wif: string,
           label?: string
         ): Account {
           ...
           wif = wif.trim();
           let pri = PrivateKey.deserializeWIF(wif);
         ...
         static importAccountByKeystore(
           password: string,
           keystore: object
         ): Account {
           const account = new Account();
           ...
           account.isDefault = true;
           // @ts-ignore
           account.encryptedKey = new PrivateKey(
             keystore.accounts[0].key
           );
           // @ts-ignore
           const privateKey = account.exportPrivateKey(
             password,
             keystore.scrypt.n
           );
         ...
         toJsonObj(): any {
           const obj = {
             ...
             salt: this.salt,
             signatureScheme: this.encryptedKey.algorithm.defaultSchema
               .label,
           };
           return obj;
         }
